...
from nltk import word_tokenize, WordNetLemmatizer, Counter
import numpy as np
...
def create_lexicon(pos_file, neg_file):
    ...
    neg_words = process_file(pos_file)
...
def string2vector(lex, review, clf):
    """
    评论转向量
    :param dict lex: 可用用的单词字典{word: position}
    :param str review: 评论
    :param list clf: 评论类型 [0]表示负面评论, [1]表示正面评论
    :return dict: 评论对应的向量
    """
...
def standardize_dataset(lex, save=''):
    """
    对数据进行标准化
    :param dict lex: 字典
    :param string save: 不为空则保存至指定路径
    :return list: 输入矩阵
    """
    dataset = []
    with open(pos_file, 'r') as f:
        lines = f.readlines()
        for review in lines:
            pos_vector = string2vector(lex, review, [1, 0])
            dataset.append(pos_vector)
    with open(neg_file, 'r') as f:
        lines = f.readlines()
        for review in lines:
            neg_vector = string2vector(lex, review, [0, 1])
...
def nn_model(data, out_num, l1_neural_num=1000):
    ...
    :type int l1_neural_num: 第一层的神经元数量
...
def train(features, out, epochs=100, batch_size=50):
    ...
    test_out = features[:test_count]
    train_out = features[test_count:]
